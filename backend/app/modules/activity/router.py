...
@router.post("/", response_model=ActivityEventSchema,
             dependencies=[Depends(require_permission('activity:manage'))])
async def create_activity(data: ActivityEventCreate, db: AsyncSession = Depends(get_db)):
...
@router.put("/{event_id}", response_model=ActivityEventSchema,
            dependencies=[Depends(require_permission('activity:manage'))])
async def update_activity(event_id: int, data: ActivityEventUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ActivityEvent).where(ActivityEvent.event_id == event_id))
    obj = result.scalar_one_or_none()
    if not obj:
        raise HTTPException(404, "活动不存在")
